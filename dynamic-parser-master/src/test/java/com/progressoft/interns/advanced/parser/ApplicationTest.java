...
import com.progressoft.interns.advanced.exception.*;
import com.progressoft.interns.advanced.parser.csvParser.*;
import com.progressoft.interns.advanced.utility.*;
...
class ApplicationTest {
    ...
    private Parser<ArrayList<ParserProperties>> parser;
    private ParsedDataUtility<ArrayList<ParserProperties>> parsedDataUtility;
    ...
    public void init(){
        ...
        //It couldn't find the file for some reason and changing this was the only way to get it to work :(
        propertiesUtility.loadProperties(getFilePath("parser.properties"));
        parser = (Parser) propertiesUtility.getClassFromProperty("parserClass");
    ...
    public void givenInvalidPath_whenParse_thenThrowParserException(){
        String fakeFileName = generateRandomString(5) + ".csv";
    ...
    public void givenValidPathWithMissingData_whenParse_thenThrowParserException() throws IOException {
        Path tempCsv = Files.createTempFile("tempCsv", ".csv");
        Random rand = new SecureRandom();
        int number = 0;
        while(number < 1) {
            number = rand.nextInt(101);
        }
        String testHeaderAndValues = "test, test2, test3";
        StringBuilder csvValues = new StringBuilder();
        for (int i = 1; i != number; i++) {
            csvValues.append(testHeaderAndValues).append("\n");
        }
        csvValues.append("Missing on line: ").append(number);
        ...
        assertEquals("Missing Data on line: " + number, parserException.getMessage());
         ...
         @Test
          public void givenValidPathButEmpty_whenParse_thenThrowParserException(){
              ParserException parserException = assertThrows(ParserException.class, () -> parser.parse(getFilePath("empty.csv")));
          ...
          public void givenInvalidColumnName_whenAnyUtilityMethod_thenThrowUtilityException(){
             ...
             //this here is the same :(
              ArrayList<ParserProperties> parsedData = parser.parse(getFilePath("sample_1.csv"));
     ...
     public void givenValidPath_whenParse_thenReturnCorrectSize(){
         assertEquals(500, parser.parse(getFilePath("sample_1.csv")).size());
         assertEquals(941, parser.parse(getFilePath("sample_2.csv")).size());
         assertEquals(1000, parser.parse(getFilePath("sample_3.csv")).size());
     }
    ...
    public void givenValidParsedData_whenGetSummation_thenReturnCorrectSummation(){
        ...
        ArrayList<ParserProperties> parsedData2 = parser.parse(getFilePath("sample_2.csv"));
        ...
        ArrayList<ParserProperties> parsedData3 = parser.parse(getFilePath("sample_3.csv"));
